...
def correct_batch_effects(
        input_data_file: str,
        max_considered_genes: int | Literal["all"] = 1000,
        save_output: bool = False,
...
if __name__ == "__main__": # ensures this code runs only when this script is executed directly, not when imported
    ...
    try:
        ...
        cluster_and_plot(os.path.join(OUTPUT_STORAGE_DIR, "CNV", "CNV_inferred_PDAC.h5ad"), ["projections"], layers=["X", "X_scVI_corrected", "X_scANVI_corrected", "X_cnv"], marker_file_path=r"C:\Users\Julian\Documents\not_synced\Github\Bachelor_thesis_pipeline\auxiliary_data\annotations\marker_genes.json", obs_annotations=["cnv_score", "cell_type", "summed_cnvs", "cancer_state", "batch"], projection="UMAP", show=True, save_output=True, verbose=True)
        # cluster_and_plot(os.path.join(OUTPUT_STORAGE_DIR, "CNV", "CNV_inferred_PDAC.h5ad"), ["projections"], layers=["X", "X_scVI_corrected", "X_scANVI_corrected", "X_cnv"], marker_file_path=r"C:\Users\Julian\Documents\not_synced\Github\Bachelor_thesis_pipeline\auxiliary_data\annotations\marker_genes.json", obs_annotations=["cnv_score", "cell_type", "summed_cnvs", "cancer_state", "batch"], projection="UMAP", show=True, save_output=True, verbose=True)
