...
if __name__ == "__main__": # ensures this code runs only when this script is executed directly, not when imported
    ...
    try:
        ...
        cluster_and_plot(os.path.join(OUTPUT_STORAGE_DIR, "batch_corrected", "batch_corrected_PDAC.h5ad"), ["projections"], layers=["X", "X_scVI_corrected", "X_scANVI_corrected"], marker_file_path=r"C:\Users\Julian\Documents\not_synced\Github\Bachelor_thesis_pipeline\auxiliary_data\annotations\marker_genes.json", obs_annotations=["cancer_state", "batch", "cell_type"], projection="UMAP", show=True, save_output=True, verbose=True)
