...
if __name__ == "__main__": # ensures this code runs only when this script is executed directly, not when imported
    ...
    try:
        ...
        output_files = aggregate_batches(os.path.join(OUTPUT_STORAGE_DIR, "cell_type_annotated"), save_output=False, verbose=True)
        correct_batch_effects(os.path.join(output_files[0], save_output=True, verbose=True, max_considered_genes="all"))
